...
final class HelperProtocolTests: XCTestCase {
    func testVersionFiveReconciliationFixturesRoundTrip() throws {
        let requestFixture = #"{"version":5,"requestId":"reconcile-1","command":"reconcilePortals","payload":{"portals":[{"portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A","portalName":"hermes","destination":{"kind":"localApp","port":8787},"desiredState":"enabled"},{"portalId":"5EA74329-3144-4BA2-925F-138D14D61FCC","portalName":"atlas","destination":{"kind":"remoteApp","scheme":"https","host":"app.example.com","port":443},"desiredState":"stopped"}]}}"#
        let responseFixture = #"{"version":5,"requestId":"reconcile-1","result":{"entries":[{"portalId":"5EA74329-3144-4BA2-925F-138D14D61FCC","outcome":"converged"},{"portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A","outcome":"startFailed"}]}}"#
    ...
    func testDecodesVersionFiveHandshakeResponse() throws {
        let fixture = Data(#"{"version":5,"requestId":"request-1","result":{"protocolVersion":5}}"#.utf8)

        let response = try JSONDecoder().decode(HelperResponse<HandshakeResult>.self, from: fixture)

        XCTAssertEqual(response.version, 5)
    ...
    func testDecodesStructuredProtocolError() throws {
        let fixture = Data(#"{"version":5,"requestId":"request-2","error":{"code":"unknownCommand","message":"unsupported command"}}"#.utf8)
    ...
    func testVersionFivePortalFixturesRoundTrip() throws {
        let portalID = UUID(uuidString: "9f55ca93-d7b3-4eab-a871-310ea576005a")!
        try assertRoundTrip(
            #"{"version":5,"requestId":"auth-1","command":"authenticatePortal","payload":{"portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A"}}"#,
            as: HelperRequest<AuthenticatePortalPayload>.self
        )
        try assertRoundTrip(
            #"{"version":5,"requestId":"cleanup-1","command":"cleanupRejectedPortal","payload":{"portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A"}}"#,
            as: HelperRequest<CleanupRejectedPortalPayload>.self
        )
        try assertRoundTrip(
            #"{"version":5,"requestId":"remove-1","command":"removePortal","payload":{"portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A"}}"#,
            as: HelperRequest<RemovePortalPayload>.self
        )
        let statusFixture = #"{"version":5,"event":"portalStatus","portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A","payload":{"state":"online","stableNodeId":"node-1","assignedName":"hermes-1","portalURL":"https:\/\/hermes-1.example.ts.net\/","addresses":["100.64.0.1","fd7a:115c:a1e0::1"],"tailnetName":"opaque-identity-do-not-display","magicDNSSuffix":"example.ts.net"}}"#
        ...
        let authenticationFixture = #"{"version":5,"event":"authenticationURL","portalId":"9F55CA93-D7B3-4EAB-A871-310EA576005A","payload":{"url":"https:\/\/login.tailscale.com\/a\/secret"}}"#
        try assertRoundTrip(authenticationFixture, as: HelperEvent<AuthenticationURLPayload>.self)
    }

    func testDecodesVersionFiveDiscoverLocalAppsResult() throws {
        let fixture = Data(#"{"version":5,"requestId":"discover-1","result":{"candidates":[{"localAppPort":3000,"processLabel":"node","suggestedPortalName":"hermes"},{"localAppPort":8787,"processLabel":"python3"}]}}"#.utf8)
