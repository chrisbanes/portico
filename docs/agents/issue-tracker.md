...
- **List issues**: `gh issue list --state open --json
  ...
  [.labels[].name], comments: [.comments[].body]}]'` with appropriate
...
- **Blocking**: GitHub's native issue dependencies are canonical. Add an edge
  ...
  issue_id=<blocker-db-id>`, using the blocker's numeric database ID. Where
  dependencies aren't available, fall back to a `Blocked by: #<n>` line.
- **Frontier query**: list the map's open children, drop children with an open
  blocker or assignee, and take the first in map order.
- **Claim**: `gh issue edit <n> --add-assignee @me`.
- **Resolve**: comment with the answer, close the issue, and append a context
